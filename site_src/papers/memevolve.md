...
- **方法流水线**(§3–4,Fig.3):
  1. **形式化 + 模块化设计空间**(§3):agentic 系统 \(M=\langle I,S,A,\Psi,\Omega\rangle\),记忆 \(\Omega\) 维护演进态 \(M_t\);任意记忆系统拆成 **\(\Omega=(E,U,R,G)\)**——**Encode**(原始经验→结构化表示)、**Store**(写入持久记忆;向量库/知识图/JSON…)、**Retrieve**(任务相关召回)、**Manage**(离线异步:巩固/抽象/遗忘)。这就是可变异的"基因型"。
  ...
  3. **双层演进**(§4.1):每轮 k 维护一组候选架构 \(\{\Omega_j\}\)(初始单例=手设计 baseline)。
     - **内层(经验演进,一阶)**:每个候选 \(\Omega_j\) 从空记忆起,沿轨迹 \(\tau\) 更新 \(M_{t+1,j}=\Omega_j(M_{t,j},\epsilon_\tau)\);跑 agent 得每轨迹反馈向量 \(f_j(\tau)\in\mathbb{R}^3\)(成功/token/延迟),聚合成 \(F_j\)。
     - **外层(架构演进,二阶)**:元算子 F 收 \(\{F_j\}\) → 选高性能候选 + 提新变体 → 下一轮候选集。F 保留 Top-K、对四组件 \((E,U,R,G)\) 修改/重组生成新架构。
  4. **Diagnose-and-Design(D&D)算子 F**(§4.2,关键):
     - **架构选择**:汇总向量 \(F_j=(\text{Perf}, -\text{Cost}, -\text{Delay})\),**非支配排序**得 Pareto rank,同 rank 内按主性能排序,选 Top-K 父代(K=1)。→ 兼顾"任务有效性 vs 资源效率"。
     - **诊断(Diagnosis)**:对父代用其执行批次 T_p 的**轨迹级证据 + replay 接口**回放,定向检查记忆行为(检索失败/无效抽象/存储低效),产出跨四组件的**结构化缺陷画像 \(D(\Omega_p)\)**。
     - **设计(Design)**:以 D 为条件,**只在模块接口的允许实现点**改(保证仍可运行),产出 S 个有效变体 \(\Omega_{p,s}=\mathrm{Design}(\Omega_p, D, s)\)。变体在编码策略/存储规则/检索约束/管理策略上不同但都符合统一接口。
