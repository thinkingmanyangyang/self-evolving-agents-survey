...
- **方法流水线**(§3–4,Fig.2):
  1. **Skill-MDP**(§3.1):经典 MDP 加一个动态**技能池 \(\Omega=\{\omega^{(1)}\dots\omega^{(K)}\}\)**(固定容量 K)当程序性记忆。状态/动作空间均自然语言。层级策略 \(\pi_\Omega(\omega_t,a_t|s_t)=\mu(\omega_t|s_t,\Omega)\cdot\pi_{LLM}(a_t|s_t,\omega_t)\)(Eq.2)。目标=最大化折扣回报(Eq.3),但**只学 \(\Omega\)**(\(\mu\)、\(\pi_{LLM}\) 冻结)→ 优化退化为优化"技能池演进算子 E"(Eq.5,\(\Omega^*=E^{(N)}(\Omega_0)\))。
  2. **技能结构**(§3.2):\(\omega\)=⟨\(I_\omega\) 激活条件 / \(\pi_\omega\) 执行过程(有序自然语言动作步)/ \(\beta_\omega\) 终止条件⟩,全自然语言、可读(对比人脑隐式,本文显式;README 称类 Options 的 Initiation/Policy/Termination)。例:StrategicPlanning。
  3. **技能选择 µ**(§3.3,两种简单实现,均冻结):(i) **按相似度** \(\omega_t=\arg\max \mathrm{Sim}(s_t,I_\omega)\)(cosine 或 LLM-as-judge);(ii) **按价值** 先 Top-k 相似再选 Q 值最高。作者强调 µ 可换更强(含 RL),不是本文重点。
  4. **技能池演进 E**(§3.4):用当前策略收集的批轨迹 \(T^{(B)}\) → \(\Omega_{new}=E(\Omega_{old},T^{(B)})\)(Eq.4),合成新技能 / 精化旧技能 / 剪掉差的。
  5. **Non-Parametric PPO**(§4)= ① **语义梯度**(§4.1):对每条调用了 \(\omega\) 的轨迹做 hindsight 归因,得 \(g_i=(g_i^I,g_i^\pi,g_i^\beta)\) 三分量自然语言建议;批量 LLM 聚合 \(\bar g_\omega\)(抽共性失败模式、滤掉冲突/轨迹专属信号);\(\omega'=\omega\oplus\bar g_\omega\)(LLM 改写=梯度上升步)。② **PPO Gate**(§4.2):把冻结 LLM 当随机策略,用旧技能 \(\omega\) 收集的轨迹算 \(\rho_t(\omega')=\pi(a_t|s_t,\omega')/\pi(a_t|s_t,\omega)\);无 value 函数,用 return-to-go 减 running baseline 估 advantage \(\hat A_t\);算 clipped surrogate \(L_{CLIP}(\omega')\),Best-of-Nc 选 \(J(\omega')\) 最大且 >0 者入池。③ **score-based maintenance**(§4.3):技能增益 \(G(\omega;\tau)\)=\(\omega\) 激活步上的平均 advantage(Eq.7);在线累积 \(\mathrm{Score}(\omega)=\Sigma G/\max(1,N)\);剪掉 \(\mathrm{Score}\le0\)、冗余、超容量(按升序剪)——baseline 随时间上升 → 形成演化压力淘汰过时技能。
...
- **假设与失效边界**:
  - 【原文】当前技能为**显式可读**形式(非隐式),隐式化方向留待 Appx F.1;\(\mu\) 和 \(\pi_{LLM}\) 固定,只演进 \(\Omega\);固定池容量 K。
  ...
  - 【推断】PPO Gate 的反事实验证在**旧技能 ω 收集的轨迹**上做,若新技能改变了"何时激活"(\(I_\omega\) 变化大),历史轨迹的状态分布未必覆盖新激活区 → off-policy 偏差;长期可能偏保守(只认历史能验证的小改)。依据:Eq.6 \(\rho_t\) 用历史 \(a_t\)、且信任域 clip 限制大偏离。
- **祛魅总结**:
  - 真贡献(硬货):① **把"PPO 信任域优化"完整迁移到文本/技能空间且零参数更新**——semantic gradient ⊕ + importance-ratio clipped Gate + advantage score 三者自洽,是方法论上的实打实创新(非简单 prompt 工程);② **极致存储压缩(816 vs 数十万 token)同时高复用高性能**,硬数据漂亮;③ Skill=⟨\(I,\pi,\beta\)⟩ 的 Options 式形式化 + Skill-MDP,给"程序性记忆"一个干净的 RL 框架。
...
|---|---|---|---|---|---|
| 环境 **reward**(经 hindsight 归因 → 自然语言"语义梯度";advantage/return-to-go 驱动 PPO Gate 与在线分);**自反思**(LLM 对轨迹做归因/聚合) | **改技能池 \(\Omega\)(自然语言技能文本:激活/执行/终止三件套)**;**base LLM、技能选择策略 \(\mu\) 全冻结、零参数更新** | **离线批量**:按批轨迹 \(T^{(B)}\) 周期性应用演进算子 E(合成/精化/剪枝);推理期 per-episode 选用技能 | **是**(纯 non-parametric;"梯度"=文本,验证用冻结 LLM 的 token 似然比,无任何反向传播) | 写入=语义梯度生成候选→PPO Gate 验证→入池;检索=\(\mu\) 按 \(I_\omega\) 相似度/价值选 Top-k;遗忘/淘汰=在线分 \(\mathrm{Score}\le0\)/冗余/超容量剪枝(FIFO 被证明会崩,必须按分剪);共享=同技能池跨 task/跨 agent 直接复用 | **隔离 + 信任域 + 打分淘汰**:base LLM 冻结天然不遗忘;PPO Gate 信任域防"演进把行为改坏";score-based pruning 淘汰过时技能、保住正增益技能(防 FIFO 式遗忘好技能) |
