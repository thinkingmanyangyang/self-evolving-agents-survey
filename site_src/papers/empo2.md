...
- **最巧的一步**:**off-policy 更新模式 (b)**——把"带 tips 采样出的动作"的 log-prob 替换成"**同一策略不看 tips** 时给该动作的 log-prob"(ℓ_no-tips = log π_θ(a|s,u)),再按优势加权更新。抽掉它,tips 的好处就只能停留在"推理时必须挂记忆",无法内化进参数→失去"脱离记忆也强"的核心卖点(也就垮回普通 Reflexion+GRPO)。这一步把"教师=挂tips策略、学生=裸策略"的**self-distillation** 显式化了。【原文 §4.2(b)】
...
- **方法流水线**(Alg.1,输入任务 u → 输出内化后的裸策略 πθ):
  ...
  5. **off-policy (b)**:把存的 log-prob 从"带 tips"换成"不带 tips"的 ℓ_no-tips=log πθ(a|s,u),即"裸策略对该动作有多自然"来加权优势——这就是 reward-guided self-distillation;
...
- **关键机制直觉**:off-policy (b) 的精髓——优势 A 已由"带 tips 采的轨迹"的真实环境 return 算出(好轨迹 A>0),但更新时却问"裸策略 πθ(a|s,u)(没看 tips)给这个动作多大概率",按 A 加权拉高/压低它。直觉=**"老师(挂 tips)示范了好动作,逼学生(裸策略)在不看小抄的情况下也学会给这个好动作高概率"**——正优势强化、负优势抑制,等价于选择性 KD。低概率 token masking 的直觉:重要性比 πθ/πθold 在 token 概率极小时分母趋零→比值爆炸→梯度爆;直接屏蔽这些 token 的贡献。【原文 §4.2】
...
| **防遗忘机制** | 训练稳定性侧:**低概率 token masking**(prob<δ 屏蔽优势项,防 off-policy 梯度爆炸/NaN)+ KL 正则向 π_ref;无显式持续学习抗遗忘组件(本文非 CL 设定)【原文 §4.2 Fig6】 |
