...
- **方法流水线**（§3,极简,核心是一个 loss 改写）：
  ...
  3. **EAFT loss(§3.3,Eq.2)**:\(L_EAFT = −Σ_t H̃_t · log P_θ(y_t|x,y<t)\)——即标准 CE 每个 token 项乘一个**归一化熵门控 H̃_t**。
  4. **门控计算(Eq.3)**:\(H̃_t = H_top-K_t / ln(K) ≈ H_top-20_t / 3.0\)——只用 **top-20** token 算熵(LLM 概率质量高度集中在头部,长尾对熵贡献可忽略),除以 ln(20)≈3.0 归一到 [0,1]。机制自调:H̃→0(模型固执/低熵)→ 权重塌、屏蔽破坏梯度;H̃→1(模型不确定/探索)→ 权重满、恢复标准 SFT 学新。
- **逐组件必要性**：
  - **熵门控(核心)**:**有强消融**——§5.1 把门控泛化为 f(H̃_t),测线性/多项式(p=2,3)/sigmoid 三类,**全部熵感知变体都胜 SFT**(Fig 5),证明增益来自"熵感知"而非具体函数。必要性证得很扎实。
