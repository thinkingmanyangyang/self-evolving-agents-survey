...
- **与最近邻工作的 Δ**：
  ...
  - **vs EWC(Fisher 加权 L2 锚定)**：EWC 软惩罚偏离旧参数;FOPNG **硬投影**到 Fisher-正交补 + 自然梯度信赖域。Δ:① FOPNG 对超参远更稳健(§5.3,α 几乎不用调、推荐 0.5;λ∈[1e-4,1e-3] 普适),EWC 超参敏感;② FOPNG 对 Fisher 估计 batch size 极鲁棒(**1 个样本估 Fisher 都能 work**,Fig 2),EWC 需大 batch。
...
- **方法流水线**（§3-§4,核心 = 一个带约束优化的闭式解 + 梯度缓冲区维护）：
  1. **存旧任务信息**:每个任务训完,在最终参数 θ*_t 处算并存 **k 个梯度**(默认 **80 个/任务**,只取 ground-truth logit 的梯度,沿用 OGD 方法),收成矩阵 G;同时维护旧任务 Fisher `F_old`(对角近似,EMA 更新 `F_old←(1−α)F_old+αF_new`)。
...
- **逐组件必要性**：
  ...
  - **自然梯度信赖域 ‖v‖_F=η**:【原文】"实测对学习稳定性是需要的"(§3.3),但**论文未给"去掉信赖域"的单独消融数字**(标出:必要性靠文字断言+稳定性观察,非独立消融)。
  ...
  - **λ 正则(稳定矩阵求逆)、α(F_old EMA 权重)**:**有消融**(附录 C,Fig 3/4)——λ∈[1e-4,1e-3] 普适、α 几乎不用调(推荐 0.5),稳健性是卖点。
- **关键机制/公式（直觉）**：
...
- **假设与失效边界**（作者 §5.4 明列）：
  - 【原文】① **需任务级梯度**(存梯度缓冲区,80/任务),受限内存/算力场景适用性差;② **训练时间随任务数线性增长**,长序列低效(比 EWC/OGD 多 40-80% wall-clock);③ Assumption 3.1:F_new 正定(可用 λ 正则放松)、G 列满秩(实践成立,存的梯度数远小于参数维)。
