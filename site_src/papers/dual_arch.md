...
══ 第三层:怎么做 + 靠不靠谱 ══
- **方法流水线**【原文 §4,Algorithm 1】:对每个新任务 k:(1)**先训 Pla-Net(可塑学习器,深窄)**——只用分类损失 \(L_{CE}=L_{plastic}\) 猛学当前任务,不管旧知识(允许遗忘)→(2)**冻结 Pla-Net 当 teacher** →(3)**再训 Sta-Net(稳定学习器,宽浅,主模型)**——用复合损失 \(L_{stable}=αL_{CE}+(1−α)L_{KD}+L_{CL}\)(Eq.3,\(α=0.5\)):\(L_{CE}\) 学硬标签、\(L_{KD}\)(KL 蒸馏 Eq.4)从冻结的 Pla-Net 软标签吸收新知识、\(L_{CL}\) 是所选 CL 方法(iCaRL/WA/DER/Foster/MEMO)自带的抗遗忘项 →(4)推理时**只用 Sta-Net**。回放 buffer 同时拼进两学习器的训练数据。
